 ...
 *
 * Wrapper class in BigCache, which contains info on access time, ttl and
 * storage.
 *
 * The {@link BigCache} will protect the r/w operation on this object by two
 * means: 1. use a striped write lock in its write operations. 2. synchronize
 * the CacheValueWrapper object in the read operations of Cache, as there may be
...
public class CacheValueWrapper {
	 ...
	 *
	 * we need to put the following restriction: 1. We will always set the time to a
	 * bigger value than the current one. 2. If the pointer has expired, don't set
	 * the value, so there won't be <em>expire</em> to <em>non-expire</em>, which is
	 * wrong
	 *
