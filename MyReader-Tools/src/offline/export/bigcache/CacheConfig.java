...
public class CacheConfig {
	 ...
	 *
	 * Only takes effect when the {@link StorageMode} is set to MemoryMappedPlusFile
	 * or OffHeapPlusFile mode, in these cases, this setting limits the max offheap
	 * memory size.
	 *
	...
	public CacheConfig setMaxOffHeapMemorySize(long maxOffHeapMemorySize) {
		if (maxOffHeapMemorySize < this.capacityPerBlock) {
			throw new IllegalArgumentException("maxOffHeapMemorySize must be equal to or larger than capacityPerBlock"
					+ this.getCapacityPerBlock());
		}
	...
	public enum StorageMode {
		PureFile, MemoryMappedPlusFile, OffHeapPlusFile,
	}
}
